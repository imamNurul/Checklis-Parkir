 ...
 * @author Imam-pc
 */
public class TambahCeklis extends javax.swing.JDialog {
...
//        }
    ...
    public void tambahCeklis(String bln, int thn){
    ...
    private void initComponents() {
        ...
        btnHapusTable.setText("Hapus Tabel");
        ...
        btnSimpanKeluar.setText("Simpan");
    ...
    private void btnHapusTableActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnHapusTableActionPerformed
        ...
        if (tableCeklisDetail.getSelectedRow() == -1) {
            ...
            tableModel.remove(tableCeklisDetail.convertRowIndexToModel(tableCeklisDetail.getSelectedRow()));
...
//                        for(int i=0;i<tableModel.getRowCount();i++){
//                            CeklisDetail item = tableModel.get(i);
//                            if(item.getJam().equals(ceklisDetail.getJam())){
//                                tableModel.remove(i);
//                                break;
//                            }
//
//                        }
                        ...
                        Logger.getLogger(TambahCeklis.class.getName()).log(Level.SEVERE, null, ex);
                }
...
//                System.out.println("bulan integer: "+ montInt);
            ...
            ceklis.setTglBuat(new java.sql.Timestamp(new java.util.Date().getTime()));
            ceklis.setBuatBy(pengguna.getKaryawan().getNama());
            ceklis.setUbahBy(pengguna.getKaryawan().getNama());
                ...
                if(lts != null){
                    ...
                    lts.stream().map((j) -> {
                        ...
                        System.out.println("get Lot dr tgl: "+j.getDaftarJadwalLot());
                         return j;
                     }).map((j) -> j.getDaftarJadwalLot()).filter((listLot) -> (listLot != null)).map((listLot) -> {
                         comboAreaPlot.removeAllItems();
                         return listLot;
                     }).forEachOrdered((listLot) -> {
                         listLot.forEach((lt) -> {
